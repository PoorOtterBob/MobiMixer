...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _get_adj(self, adj_path, device):
        graph_path = adj_path
        if not os.path.isabs(graph_path) and not os.path.exists(graph_path):
            graph_path = os.path.join(self.args.root_path, graph_path)

        adj_mx = load_adj_from_numpy(graph_path)
        adj_mx = normalize_adj_mx(adj_mx, "transition")
        supports = [
            torch.tensor(adj, dtype=torch.float32).repeat(2, 1, 1).to(device)
            for adj in adj_mx
        ]
        self.adj = [supports, supports]
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.time += time.time() - epoch_time
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                trues.append(true)
                if i % 20 == 0:
                    if self.args.down_sampling_method and not self.args.only_use_down_sampling and self.args.pred_down_sampling:
                        input = batch_x[-1].detach().cpu().numpy()
                        gt = np.concatenate((input[0, :, -1], true[0, :, -1]), axis=0)
                        pd = np.concatenate((input[0, :, -1], pred[0, :, -1]), axis=0)
                        visual(gt, pd, os.path.join(folder_path, str(i) + '.pdf'))
                        save_to_csv(gt, pd, os.path.join(folder_path, str(i) + '.csv'))
                    elif self.args.down_sampling_method and not self.args.only_use_down_sampling and not self.args.pred_down_sampling:
...
Mobility_Traffic = Exp_Long_Term_Forecast
