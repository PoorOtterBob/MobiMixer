...
from src.Embed import DataEmbedding_graph
from src.StandardNorm import Normalize
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, adj=None):
        ...
        for i, x in enumerate(x_list[0]):
            B, T, N = x.size()
            enc_out = self.enc_embedding_trend[i](x, None)
...
class Spatial_decom(nn.Module):
    def __init__(self, d_in, d_core, node_num, core_num, head_num, nndropout=0.3):
        ...
        if core_num == 0:
            raise ValueError("core_num must be greater than 0")
        else:
    ...
    def forward(self, inputs, adj=None, *args, **kwargs):
        ...
        output = torch.cat([inputs - v, v], dim=-1)
